...
- The wrapper restores to the latest common valid point across the selected database group.

### Online migration for a database group
...
### Online migration with scheduled cutover
...
- `-StatusIntervalMinutes` controls heartbeat-style monitoring output.
- `-ScheduledCutoverLocalTime` acts as the cutover gate for the database group.
- The wrapper does not cut over each database independently. It selects the latest common restorable point so the group remains aligned.

## Reporting and observability

Each wrapper run writes a report folder under `reports/migration-<timestamp>`.

Artifacts include:

- `wrapper-events.jsonl`
- `transfer-events.jsonl`
- `guided-events.jsonl`
...
- Auditability before and after cutover.
- Capturing the selected topology, actions taken, and any storage or LRS failures.

## Migration checklist

Before running either mode:

- Confirm source databases use Full recovery model.
- Validate backup chains and prefer `CHECKSUM`.
- Use compression for large backups.
- Stripe only full backups when blob size limits require it.
- Confirm the SQL Managed Instance managed identity has Read/List access to the target storage.
- Confirm the operator identity can upload backups.
- Confirm the backup root structure matches the expected instance and database folder layout.
- Confirm storage container and folder names do not include `backup`.

Before online cutover:

- Stop application writes.
- Take the final tail or log backup.
- Ensure the final uploaded log is visible to the wrapper.
- Let the wrapper cut over at the common valid recovery point.

## Limitations and design constraints

- Offline mode supports multiple source instances in one run.
- Online mode supports exactly one source instance per run.
- LRS requires a flat folder per database in storage.
- Databases remain unavailable until restore or cutover completes.
- Offline mode requires the complete restore chain to be available before restore completion.
- Online mode depends on continued log backup production and upload until cutover.

## Related files

- `examples/sample-wrapper-execution-commands.ps1`
  Current environment-specific wrapper examples.
- `pwsh/lrs-guide.md`
  Lower-level guided script notes.
- `pwsh/lrs-guided.ps1`
  Direct guided LRS control script.
- `pwsh/lrs-backup-transfer.ps1`
  Transfer engine used by the wrappers.

## Summary

Use this repository when you need a practical, script-first migration path to Azure SQL Managed Instance that:

- Handles offline multi-instance and multi-database migrations flexibly.
- Handles online multi-database migration groups with consistent recovery behavior.
- Uses managed identity for restore security.
- Tolerates MFA and claims challenges.
- Works well in outbound-only or tightly restricted enterprise network environments.
